...
if sys.argv[1:]:
    prefix = sys.argv[1]
if sys.argv[2:]:
    postfix = int(sys.argv[2])
print("Using prefix =", prefix)
print("Using postfix=", postfix)

enc_df = pd.read_csv(f"./{prefix}sender-{postfix}.csv", header=None, names=["frame_id", "enc_ts", "size"])
dec_df = pd.read_csv(f"./{prefix}receiver-{postfix}.csv", header=None, names=["frame_id", "dec_ts", "size"])
...
t3 = t3[:len(psnrs)]
t3["psnr"] = psnrs[:len(t3)]
t3["util"] = t3["psnr"] - t3["latency"] / 200
print("AVG PSNR = ", np.mean(psnrs))
print("Latency (med, p95) = ", t3["latency"].quantile(0.5), t3["latency"].quantile(0.95))
